...
const Services = () => {
  return (
    <section id="services" className="py-24 bg-surface overflow-hidden text-on-surface">
      <div className="container-tight px-4 md:px-6">
        ...
        >
          <div className="max-w-xl">
            <span className="text-primary font-black tracking-widest text-[10px] uppercase mb-4 block">Our Solutions</span>
            <h2 className="text-4xl md:text-5xl font-black mb-0 tracking-tighter">
              Elite Services for <br/> <span className="text-on-surface/20">Modern Businesses</span>
            </h2>
          </div>
          <p className="text-on-surface/40 text-lg max-w-xs font-medium">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
          {services.map((service, idx) => (
            <motion.div
              ...
              className={`${service.size} p-10 bg-white/[0.02] border border-white/5 rounded-[3rem] hover:bg-white/[0.04] hover:shadow-2xl transition-all group flex flex-col justify-between h-full`}
            >
              <motion.div 
                whileHover={{ rotate: 360, scale: 1.1 }}
                transition={{ type: "spring", stiffness: 200 }}
                className="w-16 h-16 bg-primary/10 text-primary rounded-2xl flex items-center justify-center mb-8 group-hover:bg-primary group-hover:text-surface transition-all"
              >
                {React.cloneElement(service.icon, { className: "w-8 h-8" })}
              </motion.div>
              <div>
                <h3 className="text-2xl font-bold mb-4 group-hover:text-primary transition-colors tracking-tight">{service.title}</h3>
                <p className="text-on-surface/40 leading-relaxed text-sm font-medium">{service.desc}</p>
              </div>
            </motion.div>
          ))}
        </div>
